<h1 align="center">🚀 BACH Project</h1>

<p align="center">
  💻 Multi-language Project | ⚡ C++ | Java | Python | NodeJS | JSON Data
</p>

---

<h2>📂 Project Structure</h2>

<ul>
  <li>📁 cpp → C++ source</li>
  <li>📁 java → Java source</li>
  <li>📁 python → Python source</li>
  <li>📁 node → NodeJS source</li>
  <li>📁 data → JSON data files</li>
  <li>⚙️ .vscode → VSCode settings</li>
</ul>

---

<h2>🛠 Languages</h2>

<p>
  <img src="https://img.shields.io/badge/C++-blue?style=for-the-badge&logo=c%2B%2B">
  <img src="https://img.shields.io/badge/Java-red?style=for-the-badge&logo=java">
  <img src="https://img.shields.io/badge/Python-yellow?style=for-the-badge&logo=python">
  <img src="https://img.shields.io/badge/NodeJS-green?style=for-the-badge&logo=node.js">
  <img src="https://img.shields.io/badge/JSON-black?style=for-the-badge&logo=json">
</p>

---

<h2>📊 Info</h2>

<ul>
  <li>👤 Author: S</li>
  <li>🌍 Country: Vietnam 🇻🇳</li>
  <li>💡 Project type: Learning / Multi language</li>
</ul>

---

<h2>🔥 GitHub Stats</h2>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOURNAME&show_icons=true&theme=tokyonight">
</p>

---

<h2>🐍 Contribution Snake</h2>

<p align="center">
  <img src="https://raw.githubusercontent.com/platane/snk/output/github-contribution-grid-snake.svg">
</p>

---

<h3 align="center">⭐ Thanks for visiting ⭐</h3>